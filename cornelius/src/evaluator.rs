...
pub fn eval<S: std::hash::BuildHasher>(
    expr: &RecExpr<Peg>,
    var_bindings: HashMap<&str, Peg, S>,
) -> Result<Peg, String> {
    // set up bindings
        ...
        let peg = match root {
            ...
            Phi([c, t, e]) => {
                ...
                match cond {
                    ...
                    _ => Err(
                        "Condition of phi node didn't evaluate to a ground boolean term".to_owned(),
                    ),
                }
            }
            Var(id) => Ok(self
                ...
                .get(&id)
                .ok_or(format!(
                    "Var (id={}) was not found in lookup\nbindings={:?}",
                    id, self.bindings
                ))?
                .clone()),
...
mod tests {
    ...
    fn eval_plus() -> Result<(), String> {
        ...
        assert_eq!(
            eval(&PLUS_1_3_2.parse().unwrap(), env.clone())?,
            Peg::Num(6)
        );
        assert_eq!(
            eval(&PLUS_A_B_C.parse().unwrap(), env.clone())?,
            Peg::Num(6)
        );
        Ok(())
    }

...
fn parse_peg_from_string(peg_str: String) -> Result<Peg, String> {
    let peg_str = if peg_str.starts_with('(') {
...
pub fn run_on_subjects_file(
    ...
    let subjects: Subjects =
        Subject::from_file(subj_file.to_string()).expect("Error reading subjects");
...
pub fn run_on_subjects(
    ...
    for equivalence in &subjects.node_equivalences.node_equivalences {
        let fst_id: Id = Id::from(
            equivalence
                .first
                .parse::<u32>()
                .unwrap_or_else(|_| panic!("Couldn't parse u32 {}", equivalence.first))
                as usize,
        );
        let snd_id: Id = Id::from(
            equivalence
                .second
                .parse::<u32>()
    ...
    debug!(
        "egraph total_size after deserializing: {}",
    ...
    if run_config.iter_details {
        ...
        write_iter_file(path, subj_file.to_string(), methods, &runner)
